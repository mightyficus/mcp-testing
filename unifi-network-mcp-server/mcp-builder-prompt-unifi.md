...
Build an MCP server that will interact with the UniFi Network ecosystem. I will provide a link to the API documentation below, as well as some undocumented API calls. I want to perform actions including, but not limited to, getting application info, listing local sites, performing actions on UniFi devices (listing devices, performing actions, updates, removal, etc), retrieving and manipulating networks, and controlling WiFi and policies. Create actions for any other API endpoint that you think will be useful, especially in an environment that is being used to stress test new devices and firmware that focuses on APs and WiFi. This will require an API key, and I will provide the basic API request format. This server will send API requests to the console IP, which should be provided by the user, as the IP may change when accessing different consoles. It will likely also access files in order to write reports, if requested by the user. All reports will be written in markdown unless otherwise indicated.

Here is the link to the public API as of Network version 10.1.68: https://developer.ui.com/network/v10.1.68/
Here's the example basic API request:
`curl -k -X GET 'https://192.168.10.1/proxy/network/integration/v1/sites' -H 'X-API-KEY: YOUR_API_KEY' -H 'Accept: application/json'`

Here are some of the undocumented API enpoints that I've used in a custom unifi python class:
 Base URL: https://{consoleIp}/proxy/network/api/s/{siteName}
  Method: GET
  Endpoint: .../get/setting
  Purpose: Fetch all site settings
  ────────────────────────────────────────
  Method: POST
  Endpoint: .../set/setting/super_fwupdate
  Purpose: Update firmware release channel (payload: key, firmware_channel, sso_enabled, controller_channel, _id)
  ────────────────────────────────────────
  Method: POST
  Endpoint: .../cmd/firmware
  Purpose: Trigger firmware availability check ({"cmd": "list-available"})
  ────────────────────────────────────────
  Method: POST
  Endpoint: .../cmd/devmgr
  Purpose: Trigger device upgrade — two variants:
  ────────────────────────────────────────
  Method:
  Endpoint:
  Used In:
  Purpose: - Channel upgrade: {"cmd": "upgrade", "mac": "..."}
  ────────────────────────────────────────
  Method:
  Endpoint:
  Used In:
  Purpose: - Custom URL upgrade: {"cmd": "upgrade-external", "mac": "...", "url": "..."}

If necessary, I can provide a custom Python class that I developed for interacting with UniFi devices via API.
